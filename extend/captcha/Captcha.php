...
class Captcha
{
    ...
    {
        $redis = new MyRedis();
        $key = $redis->packKey('captcha', $id, 'timestamp');
        ...
        if (password_verify(strtolower($code), $secode)) {
    ...
    public function create($id = '')
    {
        ...
        // 保存验证码
        $cache = new MyRedis();
        $key = $cache->packKey('captcha', $id, 'timestamp');
        $hash = password_hash(strtolower(implode('', $code)), PASSWORD_BCRYPT, ['cost' => 10]);
        $cache->setEx($key, $this->expire, $hash);
